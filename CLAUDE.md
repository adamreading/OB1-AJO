...
- **Inferred edges** (added 2026-05-25): edges produced by `scripts/infer-entity-edges.mjs` are written directly to the `edges` table with `metadata.source = 'inferred'`. The `/constellation` handler merges them into the response with `inferred: true`; the dashboard renders them dashed (`stroke-dasharray="4 3"`) in **light orange** (`rgba(255,176,90,…)`) instead of the violet used for co-occurrence + manual edges, so they're clearly distinguishable at any zoom level without dimming the opacity to the point of invisibility. Stroke 0.7px vs 0.8px for thought-derived edges. Inferred edges have no `thought_entity_edges` provenance row, so the `edges.support_count` trigger never touches them — they keep `support_count = 1` until explicitly modified or deleted. If the worker later extracts the same pair+relation from a thought, the trigger upserts `edges.support_count` from the new provenance rows but leaves `metadata.source = 'inferred'` intact (acceptable drift for now; manual `UPDATE edges SET metadata = metadata - 'source' WHERE …` can promote it).
- **Right-click an edge to edit** (added 2026-05-26): every line in the constellation has a fat (10px screen) invisible hit overlay underneath the visible stroke. Right-click opens `EditEdgeModal` — fetches all typed edges between the two endpoints (the visible line often collapses multiple, e.g. Adam works_on Foo + Adam member_of Foo) and lists each with a source badge (`extracted`/`inferred`/`manual`), support count, and confidence. Per-row actions: "Change relation…" (dropdown → applies as delete-old + manual-add, so the old relation lands in edge_blocklist and the worker can't re-extract it) and "✕ Remove & block" (existing DELETE behaviour). The `/entities/:id/edges` response now includes `edge_source` derived from `metadata.source` so the badge shows the right tier.
- **Manual edges** (added 2026-05-25): user-asserted edges added via `POST /edges/manual`. Written directly to `edges` with `metadata.source = 'manual'`, support_count=1, confidence=0.95. Two UI entry points: the EdgesModal on a wiki page has a "+ Add edge" inline form (search-as-you-type for the other entity, dropdown for the relation), and right-clicking any node in the constellation opens `ConstellationAddEdgeModal` with that node preset as the "from" entity. Backend respects `edge_blocklist` (returns 409 with `blocked:true` so the dashboard can prompt the user to unblock); existing edges return 200 with `existed:true` as a no-op. After write, both endpoints' most-recent thoughts are re-queued so wiki articles rebuild with the new relationship in their Relationships section. Manual edges render in the constellation as **solid** lines (same visual weight as co-occurrence) — not dashed like inferred — because they're the highest-trust tier (explicit user assertion, not LLM inference). Both `constellation_inferred_edges` and the `constellation_focus_neighbors*` RPCs now match `metadata.source IN ('inferred', 'manual')` so manual edges to non-co-occurring entities surface in focus mode too.
...
CLI: `node --env-file=.env scripts/infer-entity-edges.mjs --dry-run`, `--apply`, `--entity-id N`, `--type person,place,organization|all`, `--min-thoughts 3`, `--limit N`. Snapshots `edges` to `scripts/edge-snapshots/edges-pre-infer-<timestamp>.json` before any apply for rollback. Env knobs: `INFER_MODEL`, `INFER_RATE_MS`, `INFER_MAX_THOUGHTS`, `INFER_SNIPPET_CHARS`, `INFER_MIN_CONFIDENCE`, `INFER_NUM_CTX` (falls back to `OLLAMA_NUM_CTX`, then 32768). The Plaud curator webhook (`scripts/plaud-webhook.js`) also passes `OLLAMA_NUM_CTX` on its Ollama `options` now, so its gemma calls aren't pinned to whatever context is already resident.
...
| `WORKER_MODEL_CHAIN` | `` (empty) | Worker | Comma-separated fallback models tried when the primary `OLLAMA_MODEL` produces unparseable JSON or a token loop. Empty = no fallback. Recommended for gemma users: `qwen3:30b` catches almost all gemma loops since qwen3 doesn't share that failure mode. |
| `WORKER_LOOP_REPEAT_THRESHOLD` | `8` | Worker | Number of consecutive repetitions of a 4-30 char substring in raw model output that flags a token loop. 8 has no false positives on real captures and catches gemma loops well before the 100+ repeat range. |
...
**Worker token-loop detection + fallback chain** (added 2026-05-26): the worker's #1 failure mode with gemma4:26b is mid-string token loops — the model emits `/task_details/task_details/task_details/…` until `num_predict` cuts it off mid-loop, leaving a JSON string that's genuinely truncated and unrepairable. `detectTokenLoop()` scans the raw response tail (last 3KB) for any 4-30 char substring that repeats `WORKER_LOOP_REPEAT_THRESHOLD` times (default 8) consecutively; if found, the parse is aborted immediately and the next model in `WORKER_MODEL_CHAIN` (comma-separated, e.g. `qwen3:30b,gpt-oss:120b-cloud`) is tried. Without a chain configured the failure flows to the normal attempt-cap retry path, which is wasted time at `WORKER_TEMPERATURE=0` (deterministic same output). With `WORKER_MODEL_CHAIN=qwen3:30b` (recommended for gemma users), gemma loops trigger an immediate qwen retry and the thought completes on the same poll cycle. Loop detection runs BEFORE `parseJsonObject`, so it short-circuits the four repair strategies for the obvious unrecoverable case.
