...
-- AJO compatibility: earlier AJO installs already created
-- entity_extraction_queue for local Work/Personal classification with only
-- thought_id/status/last_error timestamps and a terminal status of "done".
-- CREATE TABLE IF NOT EXISTS is a no-op in that case, so explicitly upgrade
-- the existing queue to the graph/wiki contract used by the local worker and
-- wiki compiler.
...
BEGIN
  FOR r IN
    SELECT conname
    FROM pg_constraint
    WHERE conrelid = 'public.entity_extraction_queue'::regclass
      AND contype = 'c'
      AND pg_get_constraintdef(oid) ILIKE '%status%'
  LOOP
    EXECUTE format('ALTER TABLE public.entity_extraction_queue DROP CONSTRAINT %I', r.conname);
  END LOOP;
  ...
  IF NOT EXISTS (
    ...
    FROM pg_constraint
    WHERE conrelid = 'public.entity_extraction_queue'::regclass
      AND conname = 'entity_extraction_queue_status_check'
  ) THEN
    ALTER TABLE public.entity_extraction_queue
      ADD CONSTRAINT entity_extraction_queue_status_check
...
-- ============================================================
-- 10. WIKI PAGES
--     Persistent store for generated wiki pages (entity and topic).
--     The wiki compiler writes here after generating each page.
--     manually_edited=true protects hand-edited pages from compiler
--     overwrites. The REST API PUT endpoint sets this flag on save.
-- ============================================================
