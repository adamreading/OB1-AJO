...
Run the unified start script from the project root. This launches all four local processes inside a single Windows Terminal window using **psmux** with a 2×2 pane layout.
...
|------|---------|-------------|
| Top-left | **Dashboard** (Next.js) | http://localhost:3010 (and reachable via Tailscale tailnet hostname for the mobile app) |
| Top-right | **Plaud Webhook** (curator) | http://127.0.0.1:4001/webhook — receives Applaud `transcript_ready` events |
...
*   **Wiki view modes**: Two views toggleable from the header. **Graph** (default) — collapsible constellation hero + 2-col body (TLDR/Detailed left, Relationships/Open Questions/Curator Note right). Bare entity names auto-link in prose against the entity-name→slug map, so the LLM emitting "Adam Ososki" as plain text still becomes a clickable link. **List** — alphabetical sidebar (virtualised via `@tanstack/react-virtual`) + side-panel detail. Filter chips on the sidebar are dynamic from `/entity-types`.
*   **Wiki article style**: The regen prompt produces a short TLDR plus a narrative Detailed section (paragraphs, not bullet lists). Detailed word count is scaled to material density: `max(300, min(1000, 300 + sqrt(link_count) * 100))` — treated as a ceiling with a "do not pad" rule, so an entity with 3 linked thoughts gets ~470 words while one with 80+ links can use the full 1000. Legacy pages compiled before this change still have Summary/Key Facts/Timeline sections — the dashboard parser handles both shapes during the regen window.
...
*   **Plaud capture triage (local-curator, 2026-05-17 onward)**: Plaud transcripts arrive via Applaud → `scripts/plaud-webhook.js` (the curator). The webhook applies `processor_prompt/entity_corrections.json` STT fixes, parses `---ENTRY---` blocks (TYPE/CONTEXT/ENTITIES/SEARCH_HINTS/body/ACTIONS), searches Open Brain via REST per `SEARCH_HINTS` and `ENTITIES`, asks local Qwen3:30b to decide IGNORE / UPDATE / CAPTURE, and writes via `/capture-pending` so everything lands in the dashboard `/review` panel for manual approval. Uncertain decisions and `flag_only` STT triggers raise queries in `processor_prompt/OPEN_QUESTIONS.md`; answered queries get reapplied on the next webhook payload. Runs locally on the 5090 at £0 ongoing — replaces the abandoned Cowork-scheduled-Claude design from 2026-05-16. The MCP `auto_review` flag still exists on `capture_thought` / `update_thought` as a fallback path for chat clients that want to push manually; the webhook bypasses MCP and calls REST directly.

---

*AJO fork of Open Brain Pro. Last updated May 2026 — wiki notes/rename/alias/type/delete UI, entity-type sidebar filter, deep-linking, heuristic scoring, configurable audit threshold, Kanban card-to-card drag, wiki-wipe + score-thoughts scripts, ChatGPT search/fetch compatibility tools, wiki MCP tools (search_wiki/read_wiki_page/get_entity_connections), improved edge extraction (typed relation vocabulary + entity-type constraints), persona synthesis script, MCP v1.5.0 (get_context_brief/resume/list_action_items/capture_review + auto_review flag on capture_thought/update_thought), Actions dashboard page with Done + Promote to Kanban triage, Tailscale-friendly mobile-portrait dashboard, local-curator Plaud capture pipeline (Applaud + plaud-webhook.js + Qwen3 curator + entity corrections + open-questions journal).*
